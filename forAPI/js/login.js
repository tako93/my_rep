...
const onSubmit = async (event) => { //1. ცვლადები შეიქმნება submitის დროს და მემორი აღარ გადაიტვირთება
    ...
    const result = await window.API.login(inputEmail.value, inputPassword.value) //5. ვიყენებთ ლოგინის იმ პარამეტრებს რაც მეორე ჯს ფაილში განვსაზღვრეთ. window.API. ით მივწვდით login-ს ანუ იმ ფაილში განსაზღვრულ ფუნქციას და მივანიჭეთ შესაბამისი პარამეტრები. ამით მივიღეთ ტოკენი რომელიც შეინახა ლოქალ სთორიჯში
...
function navigateToDasboard(token) {
    ...
    localStorage.setItem(window.USER_TOKEN_KEY, token);
